...
<img style="margin-bottom: 30px;max-width:100%;" src="https://twooctobers.com/wp-content/uploads/2020/07/bitsbox-1-e1595613433370.png">
...
<img style="display: inline-block; max-width: 100%;height:48px;" src="https://twooctobers.com/wp-content/uploads/2020/07/bitsbox-1-e1595613433370.png">
<span style='color:white;font-size:50px;'>BROWSER</span>
<p>
...
`,
    ...
    #handleURI(url) {
        if (!window.navigator.onLine && !RegExp("^bitsbox:/*").test(url)) return ["data:text/html," + encodeURI(this.protocols.offline), "Not Connected"];
        ...
        if (/^\S*:/i.test(url)) {
            for (let i = 0; i < Object.keys(this.protocols).length; i++) {
                ...
                if (RegExp("^bitsbox:/*" + protocol + "$").test(url)) {
                    return [
